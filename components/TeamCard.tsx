  ...
  return (
    <Link href={`/team/${manager.slug}`} style={{ textDecoration: 'none' }}>
      <div
        ...
        onMouseEnter={e => {
          ;(e.currentTarget as HTMLElement).style.borderColor = '#4f7ef0'
          ;(e.currentTarget as HTMLElement).style.background = '#1e2235'
        }}
        onMouseLeave={e => {
          ;(e.currentTarget as HTMLElement).style.borderColor = '#2e3347'
          ...
          <div style={{ height: 5, background: '#2e3347', borderRadius: 3, overflow: 'hidden' }}>
            <div
              style={{
                height: '100%',
                width: `${capPct}%`,
                background: cap_space <= 0 ? '#f0614f' : cap_space <= 5 ? '#f0c040' : '#4f7ef0',
                borderRadius: 3,
