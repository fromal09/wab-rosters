  ...
  return (
    <>
      ...
      <header style={{ background: '#fff', borderBottom: '1px solid #e4e7ec', padding: '0 16px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', position: 'sticky', top: 0, zIndex: 50 }}>
        ...
        {/* Mobile menu */}
        {menuOpen && (
          <div className="nav-mobile" style={{ flexDirection: 'column', padding: '8px 0 12px', borderTop: '1px solid #e4e7ec', gap: 0 }}>
            <div style={{ padding: '8px 16px' }}>
              <GlobalSearch onSelect={(p) => { setSelectedPlayer(p); setMenuOpen(false) }} />
            </div>
            {navLinks.map(l => (
